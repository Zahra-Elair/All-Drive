...
import { GetTabs } from "../../data/navigation";
...
const SidebarLeft = ({ isOpen }: Props) => {
  ...
  console.log(isOpen);
  return (
    <div className={cn(" w-0 lg:w-72 bg-red-200  relative ")}>
      <div
        className={`absolute top-0 transition-all will-change-transform duration-500 ease-in-out bg-white h-full border-r p-12 w-72 lg:static ${
        ...
        <div className=" flex flex-col h-full w-full">
          <div className="flex flex-col gap-4 w-full">
            {TABS.map((tab, index) => {
              ...
              return <TabLink key={index} icon={tab.icon} label={tab.label} path={tab.path} active={active} />;
            })}
          </div>
